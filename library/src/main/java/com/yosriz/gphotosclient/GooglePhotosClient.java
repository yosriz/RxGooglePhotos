...
public class GooglePhotosClient {
    ...
    /**
     * Creates a service without displaying UI, returns a {@link GooglePhotosService} for user who
     * is signed in to the app.  If no user is signed in, try to sign the user in without displaying
     * any user interface. <p>In case of no user signed in, onError will be called with {@link
     * SignInRequiredException}, for getting access use {@link #createServiceWithSignIn(FragmentActivity)}.</p>
     *
     * @param activity associated activity
     * @return GooglePhotosService service when success
     */
    public Single<GooglePhotosService> createServiceSilently(@NonNull FragmentActivity activity) {
        return googleSignIn.getTokenSilently(activity)
                .map(signInAccount -> new GooglePhotosService(createRetrofit(signInAccount.getToken()), signInAccount.getAccount()));
    }

    /**
     * Creates a service while UI might be displayed, returns a {@link GooglePhotosService} for user
     * who is signed in to the app. <p>If no user is signed in, account selection and permission
     * dialogs will be displayed.</p>
     *
     * @param activity associated activity to display UI on top
     * @return GooglePhotosService service when success
     */
    public Single<GooglePhotosService> createServiceWithSignIn(@NonNull FragmentActivity activity) {
        return googleSignIn.getToken(activity)
                .map(signInAccount -> new GooglePhotosService(createRetrofit(signInAccount.getToken()), signInAccount.getAccount()));
    }

    public Completable signOut(@NonNull FragmentActivity activity) {
        return googleSignIn.signOut(activity);
    }
